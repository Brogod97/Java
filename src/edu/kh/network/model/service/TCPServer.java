...
public class TCPServer {
	...
	public void serverStart() {
		...
		PrintWriter pw = null; // 출력용 보조 스트림 변수 ( **BufferedWriter 도 있음 )
		// PrintWriter에는 println, print, printf 등 다양한 출력 함수를 제공함으로써 파일 출력을 편하게 해줌
		...
		try {
			...
			String clientIP = clientSocket.getInetAddress().getHostAddress(); // IP 주소 형태 만들기
			
			System.out.println(clientIP + "가 연결을 요청함...");
			
			// 5. 연결된 클라이언트와 입출력 스트림 생성
			is = clientSocket.getInputStream();		// clientSocket에 제공하는 스트림
			os = clientSocket.getOutputStream();	// clientSocket에 제공하는 스트림
			
			// 6. 보조 스트림을 통해 성능 개선
			br = new BufferedReader( new InputStreamReader(is) );
			// InputStreamReader() : 바이트 기반 스트림과 문자기반 스트림 연결에 사용하는 스트림
			
			pw = new PrintWriter(os); // 따로 추가 연결용 필요 없음
			
			// 7-1 ) 스트림을 통해 읽고 쓰기
			pw.println("[서버 접속 성공]");
			pw.flush(); // 스트림에 있는 내용을 모두 밀어냄.
			// close()도 밀어내는 기능을 하는데, 굳이 flush()를 쓰는 이유는
			// 스트림을 닫지 않은 상태에서 내용을 내보내고 싶은 경우가 있을 수 있기 때문
			// close()의 밀어내기 기능은 궁극적으로 끊어내는 기능에 부가적인 것
			...
			// null.close()가 되지 않도록 열려있는 파일만 close() 진행
			
			try {
				// 보조 스트림 close시 연결된 기반 스트림(is, os)도 같이 close됨
				if(pw != null) pw.close();
				if(br != null) br.close();
