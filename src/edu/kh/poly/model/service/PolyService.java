...
public class PolyService {
	...
	public void ex1() {
		...
		// Error : The method setBatteryCapacity(int) is undefined for the type Car
		// Car 타입에는 setBatteryCapacity 메소드가 정의되어있지 않다.
		...
		// Error : The method setDiscountOffer(int) is undefined for the type Car
		...
		//			같은 객체 => 부모타입으로 바꿈
		//			부모 타입 참조 자료형
		// + 다형성을 적용 => (부모 타입 참조) 자료형의 변수를 하나의 묶음으로 다루는 것
		
		Car[] arr = new Car[3]; // 부모타입 참조변수 배열 선언 및 할당
								// 각 배열 요소 Car 타입 참조 변수
	...
	public void ex2() {
		...
		// Car를 대상으로 처음부터 검사하면 모두 true여서 원하는 대로 동작하지 않게 됨
		for(int i = 0; i < arr.length; i++) {
	...
	public void ex3() {
		...
		// c1.getBetteryCapacity(); -> c1이 부모 타입이라 접근 불가능
		System.out.println( ((Tesla)c1).getBatteryCapacity() );
		
		// 주의!
		// "." 연산자가 (Tesla) 형변환 연산자보다 우선순이가 높음.
		// 먼저 c1을 Tesla로 형변환 해주고, getBatteryCapacity 호출해야 함
		
		// <효율적인 다운캐스팅 방법>
		// - 얕은 복사(주소 복사)를 이용한다.
		Tesla t1 = (Tesla)c1;
				// (Tesla)를 빼면 c1은 Car 타입임
		// 자식참조변수로 부모객체참조? (반대는 업캐스팅으로 가능)
		// c1이 참조하는게 Tesla라는걸 알기 때문에 Tesla로 강제 형변환 가능
		// Tesla = Tesla 같은 자료형 연산 가능하니 같은 주소를 복사시켜 버림. (얕은 복사)
	...
	public void ex4() {
		...
		// 	  Spark 참조변수로 Tesla를 참조하려고 하면 예외가 발생됨
		// -> 다운 캐스팅 주의사항 : 강제형변환을 잘못하면 안됨
		
		// 문제점 1을 방지하기 위해 instanceof와 같이 사용
		if(c1 instanceof Spark) {
			Spark s1 = (Spark)c1; // 조건이 true면 다운 캐스팅
	...
	// createCar()
	// 전달받은 매개변수에 따라서 Car 또는 자식 객체를 생성
	// 생성된 객체의 주소를 반환
	public Car createCar(int num) {
		...
		// 반환형이 Car이지만
		// case가 2, 3번이면 Car의 자식 객체의 주소가 반환된다.
	...
	public void ex5() {
		...
		// 실행해보면 자식(Spark)의 toString()이 호출되는 것을 확인할 수 있음
		// -> 컴파일시에는 부모(Car)와 바인딩 == [정적 바인딩]
		// -> 실행시에는 자식(Spark)이 오버라이딩된 메소드와 바인딩 == [동적 바인딩]
